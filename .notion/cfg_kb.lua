...
Neg_kb.WMPlex_toplevel = {
    ...
    kpress("M4+M",     "spawn('~/bin/scripts/rofi_xprop' .. ' ' .. _chld:xid())"),
    ...
    submap("M1+E",{
        ...
        kpress("d",    "spawn('~/bin/scripts/dzen-dict')"),
        kpress("i",    "spawn('~/bin/pls -output')"),
        kpress("m",    "app.byclass('~/bin/scripts/run_mutt', 'mutt')"),
        kpress("n",    "app.byinstance('nicotine.py || nicotine', 'Nicotine.py', 'nicotine.py')"),
        kpress("o",    "spawn('~/bin/pls -sink')"),
        kpress("p",    "spawn('st pulsemixer')"),
        kpress("q",    "spawn('~/bin/mpd_sel.sh')"),
        kpress("Sh+d", "spawn('~/bin/scripts/exec_demo.sh')"),
        kpress("Sh+k", "spawn('~/bin/scripts/toggle_keynav')"),
        kpress("Sh+o", "spawn('~/bin/pls -vol')"),
        kpress("Sh+r", "ranger()"),
        kpress("l",    "radare2()"),
        kpress("Sh+g", "gdb()"),
        kpress("s",    "spawn('skype')"),
        kpress("t",    "spawn('urxvt')"),
        kpress("v",    "spawn('vmware')"),
        kpress("w",    "spawn('~/bin/webcam')"),
    }),
}
