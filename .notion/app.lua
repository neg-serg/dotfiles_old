...
function app.match_class(class, instance)
    ...
    local result = {}
    local offset = 0
    local currwin = notioncore.current()
    notioncore.clientwin_i(function (win)
...
function app.byname(prog, name, where)
    local win = notioncore.lookup_clientwin(name)
    if win then
        notioncore.defer(function () win:goto_focus() end)
    else
        if where then
            notioncore.exec_on(where, prog)
        else
            notioncore.exec(prog)
        end
    end
end
...
function app.match_class_withtag(class, instance, tag)
    ...
    local currwin = notioncore.current()
    notioncore.clientwin_i(function (win)
        local winprop = notioncore.getwinprop(win)
